...
-- Existing public wishes that were waiting for moderation become visible
...
begin
  ...
  values (
    ...
    p_visibility,
    case
      when p_visibility = 'public' then 'approved'::public.wish_status
      else 'pending'::public.wish_status
    end,
    p_locale,
    p_submitter_hash,
    v_now,
    v_now,
    case
      when p_visibility = 'public' then v_now
      else null
    end
  )
  returning *
...
comment on function public.submit_wish_atomic(
  ...
  'Atomically rate-limits wishes and immediately approves public submissions.';
